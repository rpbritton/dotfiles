...
IP_ADDRESS=$(lxc info "$CONTAINER_NAME" | yq '.Resources["Network usage"]["eth0"]["IP addresses"]["inet"]' | cut -d "/" -f 1)
...
echo "Copying git drop-squashed utility"
ssh $SSH_HOST "mkdir -p ~/.local/bin"
scp ~/bin/git-drop-squashed $SSH_HOST:~/.local/bin/git-drop-squashed
